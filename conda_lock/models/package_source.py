"""
Conda lock supports two kinds of credentials used for channels and pip repositories
...
class PackageSourceUrl(BaseModel):
    raw_url: str
    env_var_url: str
    ...
    @classmethod
    def from_string(cls, value: str) -> "PackageSourceUrl":
...
class PackageSource(ZeroValRepr, BaseModel):
    model_config = ConfigDict(frozen=True)

    url: str
    used_env_vars: FrozenSet[str] = Field(default=frozenset())

    def __lt__(self, other: "PackageSource") -> bool:
    ...
    def from_string(cls, value: str) -> Self:
        if "://" in value:
            # url like string
            return cls.from_conda_url(PackageSourceUrl.from_string(value))
        else:
            # this is a simple url
            return cls(url=value, used_env_vars=frozenset([]))

    @classmethod
    def from_conda_url(cls, value: PackageSourceUrl) -> Self:
...
def _env_var_normalize(url: str) -> PackageSourceUrl:
    """
