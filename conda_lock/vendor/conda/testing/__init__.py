...
def conda_move_to_front_of_PATH():
    if 'CONDA_PREFIX' in os.environ:
        from conda_lock.vendor.conda.activate import (PosixActivator, CmdExeActivator)
        if os.name == 'nt':
...
def conda_check_versions_aligned():
    ...
    from conda_lock.vendor import conda
    ...
    for pe in os.environ.get('PATH', '').split(os.pathsep):
        if isfile(join(pe, git_exe)):
            try:
                ...
                from conda_lock.vendor.conda.auxlib.packaging import _get_version_from_git_tag
